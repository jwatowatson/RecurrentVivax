...
#=======================================================================
# This function plots allele frequencies used to calculate theta
# Since allele frequencies may differ to the input of ColorPlot_MSdata, 
# to avoid misleading colour schemes, they are not coloured 
# Author: Aimee Taylor 
#=======================================================================
Plot_Fs = function(MS_data, Fs){
  markers = names(Fs)
  no_markers = length(Fs) 
  par(mfrow = c(3,3), mar = c(4,4,0.1,0.1))
  for(i in 1:no_markers){
    counts = Fs[[i]]
    barplot(counts, col = 'gray', xlab = '', yaxt = 'n', las = 2, 
            ylab = 'Frequency', cex.axis = 1, cex.names = 0.6)
    axis(side = 2, las = 2)
    title(xlab = markers[i], line = 2)
  } 
}