...
censored_ind = Combined_Time_Data$Censored == 1 # indexed censored rows in Combined_Time_Data to prevent over counting rows
uncensored_patientids = Combined_Time_Data[!censored_ind, 'patientid']
All_VHX_epi_count = table(uncensored_patientids[grepl('VHX',uncensored_patientids)])
All_BPD_epi_count = table(uncensored_patientids[grepl('BPD',uncensored_patientids)])
...
par(mfrow = c(1,2), family = 'serif', pty = 's') # For plots



# VHX data set
no_of_epi_per_person_typed = All_VHX_epi_count[names(All_VHX_epi_count) %in% names(Typ_VHX_epi_count)][names(Typ_VHX_epi_count)]
no_of_typed_epi_per_person_typed = Typ_VHX_epi_count 
X0 = length(no_typed_person_typed)
X1 = X0 - sum(no_of_epi_per_person_typed == no_of_typed_epi_per_person_typed) # 90% completely typed
X2 = range((no_of_epi_per_person_typed - no_of_typed_epi_per_person_typed)[no_of_epi_per_person_typed != no_of_typed_epi_per_person_typed]) 
X3 = sum(no_of_epi_per_person_typed - no_of_typed_epi_per_person_typed)
plot(x = as.numeric(no_of_epi_per_person_typed), y = as.numeric(no_of_typed_epi_per_person_typed), 
     xlab = 'Number of episodes per person', ylab = 'Number of typed episodes per person', main = 'VHX',
     ylim = range(no_of_epi_per_person_typed), xlim = range(no_of_epi_per_person_typed ))
abline(a = 0, b = 1, lty = 'dotted')
writeLines(sprintf('Of %s of %s VHX individual/s selected for genotyping: %s to %s of their episodes were not typed (%s episodes summing over the %s individuals)',
                   X1,X0,X2[1],X2[2],X3,X1))

# BPD data set
no_of_epi_per_person_typed = All_BPD_epi_count[names(All_BPD_epi_count) %in% names(Typ_BPD_epi_count)][names(Typ_BPD_epi_count)]
no_of_typed_epi_per_person_typed = Typ_BPD_epi_count
X0 = length(no_of_typed_epi_per_person_typed)
X1 = X0 - sum(no_of_epi_per_person_typed == no_of_typed_epi_per_person_typed)
X2 = range((no_of_epi_per_person_typed - no_of_typed_epi_per_person_typed)[no_of_epi_per_person_typed != no_of_typed_epi_per_person_typed]) # missing 1 if missing
X3 = sum(no_of_epi_per_person_typed - no_of_typed_epi_per_person_typed)
plot(x = as.numeric(no_of_epi_per_person_typed), y = as.numeric(no_of_typed_epi_per_person_typed), 
     xlab = 'Number of episodes per person', ylab = 'Number of typed episodes per person', main = 'BPD',
     ylim = range(no_of_epi_per_person_typed), xlim = range(no_of_epi_per_person_typed))
abline(a = 0, b = 1, lty = 'dotted')
writeLines(sprintf('Of %s of %s BPD individual/s selected for genotyping: %s to %s of their episodes were not typed (%s episodes summing over the %s individuals)',
                   X1,X0,X2[1],X2[2],X3,X1))
