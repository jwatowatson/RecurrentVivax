...
if(CREATE_PLOTS){
  ...
  CI = cbind(apply(Thetas_full_post[reLapse_ordered$i,grep('L',colnames(Thetas_full_post)),],
  ...
  for(i in 1:length(reLapse_ordered$x)){
    if(diff(CI[i,]) > 0.005) arrows(i,CI[i,1],i,CI[i,2], length = 0.02,angle = 90, 
                                    code = 3, col='blue')
  }
...
MS_pooled$L_or_C_state = MS_pooled$ClusterID = MS_pooled$TotalEpisodes = NA
...
for(i in 1:length(ids)){
  ...
  MS_pooled$NumberClusters[MS_pooled$ID==id] = components(Graphs[[i]])$no
  # Add the membership of each cluster
  MS_pooled$ClusterID[MS_pooled$ID==id] = components(Graphs[[i]])$membership
  MS_pooled$L_or_C_state[MS_pooled$ID==id] = duplicated(components(Graphs[[i]])$membership)
}
```
...
for(i in 1:length(ids)){
  ...
  drug_col = as.numeric(
    Combined_Time_Data$arm_num[Combined_Time_Data$patientid==id][1] == 'CHQ/PMQ') + 3 
  lines(c(0,Combined_Time_Data$FU_time[Combined_Time_Data$patientid==id][1]), 
        c(i,i), lty=1, 
        lwd=.5, col= mycols[drug_col])
  if(Neps > 0){
    cols = mycols[as.numeric(MS_pooled$L_or_C_state[ind])+1]
