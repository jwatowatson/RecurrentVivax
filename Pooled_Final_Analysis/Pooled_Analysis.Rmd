...
Under a uniform distribution over $n$ alleles, the probability of seeing the same alleles twice by chance is simply $1 - \frac{1}{n(n-1)}$ (1 minus the probability of seeing two different ones).
Therefore, to calculate the effective allele size, we solve the quadratic $x = 1 - \frac{1}{n(n-1)}$, which has a real solution at $\frac{1+\sqrt{1+4(\frac{1}{1-x})}}{2}$.

```{r}
N = 10^6
Effective_Allele_size = list()
for(ms in MSs_all){
  draw1 = sample(x = names(Fs_Combined[[ms]]), replace = T, size = N, 
                 prob = Fs_Combined[[ms]])
  draw2 = sample(x = names(Fs_Combined[[ms]]), replace = T, size = N, 
                 prob = Fs_Combined[[ms]])
  # calculate the probabilty of seeing different alleles by simulation
  x = 1-sum(draw1 == draw2)/N
  writeLines(sprintf('The effective allele size for %s is %s',
                      ms, 
                     round( (1 + sqrt(1+4*(1/(1-x))))/2,2)))
  Effective_Allele_size[[ms]] = round( (1 + sqrt(1+4*(1/(1-x))))/2,1)
}
```
