...
# Also, Thetas_full_post has redundant coolumn names
```
...
if(CREATE_PLOTS){
  ...
  for(ms in MSs_all){ # As bars
    ...
    title(ylab='Frequency', xlab= sprintf('Repeat length (motif length = %s)', MSs_Motifs[[ms]]), line = 3)
    for(k in 1:ncol(xs)){
      lines(rep(k,2), quantile(xs[,k],probs = c(0.025,0.975)))
    }
    points(1:length(Fs_Combined[[ms]]),Fs_Combined[[ms]],pch=18)
...
Time_Estimates_1 = filter(Mod3_ThetaEstimates, Episode_Identifier %in% thetas_9MS$Episode_Identifier)
...
if(CREATE_PLOTS){
  ...
  for(state in states){
    ...
    CI = t(apply(X, 1, quantile, probs = c(0.025, 0.975)))
    
    # I don't think the bars at the end of CIs are necessary
    for(i in 1:length(ordered$x)){
      if(diff(CI[i,]) > 0.005) segments(x0 = i, y0 = CI[i,1], x1 = i, y1 = CI[i,2], 
...
MS_pooled_summary$Plotting_pch_Values = 
  as.numeric(mapvalues(MS_pooled_summary$L_or_C_state, from = c('L','Uncertain','I'), to = c(21,15,24)))
...
if(CREATE_PLOTS){
  ...
  plot(NULL, ylab = '', xlab = '',
       ...
       ylim = c(1,length(ids)),
       xaxt='n', yaxt='n')
  mtext(text = 'Days from start of study', side = 1, line=2.5, cex=1)
  axis(1, at = seq(0,370, by=60), cex.axis=1)
  ...
  legend('topright', col = c(mycols_states_fg[c(3,2,1)]), 
         pch = c(1,15,17), cex = 1, bty = 'n', y.intersp = 0.75, title = 'Classification',
...
if(CREATE_PLOTS){
  ...
  plot(NULL,
       ...
       yaxt='n', ylab='', xlab = '')
  axis(2, at = c(0,.5,1))
  title(ylab = 'Relapse posterior probability', xlab = 'Rank', line = 3)
  ...
  legend('right', col = drug_cols2[2:3], 
         pch = 20, cex = 1, bty = 'n', 
         legend = c('No PMQ','PMQ'))
  ...
  plot(NULL, 
       ...
       xlab = '',ylab = '',yaxt='n')
  axis(2, at = c(0,.5,1))
  title(xlab = 'Days since last episode', ylab = 'Relapse posterior probability', line = 3)
...
```{r}
APC_MS_data = APC_MS_data[which(APC_MS_data$Episode_Identifier %in% rownames(Inflated_Results)),]
village_VHX = readxl::read_excel('../../Genotyping/Data/VHX_villages.xls')
APC_MS_data = APC_MS_data[intersect(grep('VHX',APC_MS_data$ID1), grep('VHX',APC_MS_data$ID2)),]
APC_MS_data = APC_MS_data[!duplicated(APC_MS_data$ID),]
```
