...
RUN_MODELS_FULL_POSTERIOR_SIMPLE = F    # can do on laptop (3 hours)
# runs the model for 100 random draws from the Time model posterior on those that cannot be directly computed
RUN_MODELS_FULL_POSTERIOR_INFLATED = F  # cluster is better (2 days?)
# Does the false positive discovery estimation
RUN_MODELS_FALSE_POSITIVE = F           # only cluster!! (2 weeks?)
# generates plots
CREATE_PLOTS = T
...
writeLines(sprintf('From BPD trial there are %s individuals with total of %s episodes typed',
...
This isn't working as I need to add timeSinceLastEpisode in data processing (removed it)
