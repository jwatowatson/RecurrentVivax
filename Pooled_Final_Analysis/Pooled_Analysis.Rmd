...
check <- function(n_in){
  ...
  return(c(n_in = n_in, x_same_cal = x_same_cal, x_same_num = x_same_num, n_out_num = n_out_num))  
}
X = sapply(c(2, 2:10), check)
plot(x = X[2,], y = X[3,], 
...
for(ms in MSs_all){
  ...
  x = mean(draw1 == draw2)
  n = 1/x
  writeLines(sprintf('The effective marker cardinality for %s with %s observed alleles is %s', ms, n_obs_alleles, round(n,2)))
  Effective_Allele_size[[ms]] = n
}

# # Check roughly the same
# check(Effective_Allele_size[[ms]])
