...
Bug in the next bit so commented out: I don't get a bug

```{r VHX_typed_untyped}
#=================================================================
# Is there a significant difference in episode counts between VHX 
# that where genetically typed and not?
#=================================================================

# Condition on CQ (comment out next three lines of code to remove condition)
...
# plot store was generated for barplot (replaced by histogram)
plot_store = array(dim = c(2, max_rec), dimnames = list(c('Untyped','Typed'), 1:max_rec))
...
theta_hat = median(x1) - median(x2) # More episodes per person in untyped
...
Normal_95CI = c(theta_hat - 1.96*se, theta_hat + 1.96*se)  # Not significant
...
Under a discrete uniform distribution over $n$ alleles, the probability of seeing the same alleles twice by chance is simply $1 - \frac{1}{n(n-1)}$ (1 minus the probability of seeing two different ones). Therefore, to calculate the effective marker cardinality, we solve the quadratic $x = 1 - \frac{1}{n(n-1)}$ for n, which has a real solution at $n = \frac{1+\sqrt{1+4(\frac{1}{1-x})}}{2}$.
...
for(ms in MSs_all){
  ...
  # calculate the probabilty of seeing different alleles by simulation 
  x = 1-sum(draw1 == draw2)/N
  n = (1 + sqrt(1+4*(1/(1-x))))/2 
  writeLines(sprintf('The effective marker cardinality for %s is %s', ms, round(n,2)))
  Effective_Allele_size[[ms]] = round(n,1)
}
```

James, above x is mathematically defined as the probability of seeing two the same. In the code it is the probability of them being different (i.e. 1- sum(draw1 == draw2)/N = mean(draw1 != draw2)). If let $x = \frac{1}{n(n-1)}$, then $n^2 - n - \dfrac{1}{x} = 0$ and $n = \frac{1+\sqrt{1+\frac{4}{x}}}{2}$. In any case, after much de-bugging I realise $x = \frac{1}{n(n-1)}$ is NOT the probability of two things being different (it decreases as n increases). The probability of two things being different under a discrete uniform is simply $\frac{1}{n} = n \times \frac{1}{n^2}$. I've checked this numerically below, and recalculated the effective allele sizes. 

```{r}
# Check function meets expectation 
check_AT <- function(n_in){
  draw1 = sample(x = n_in, replace = T, size = N)
  draw2 = sample(x = n_in, replace = T, size = N)
  x_same_num = mean(draw1 == draw2)
  x_same_cal = 1/n_in
  n_out_num = 1/x_same_num
  return(c(n_in, x_same_cal, x_same_num, n_out_num))  
}
X = sapply(c(2, 2:10), check_AT)
plot(x = X[2,], y = X[3,], 
     ylab = 'Prob same by calculation', xlab = 'Prob same numerically')
abline(a = 0, b = 1)
...
for(ms in MSs_all){
  ...
  x = mean(draw1 == draw2)
  n = 1/x
  writeLines(sprintf('The effective marker cardinality for %s with %s alleles is %s', ms, n_obs_alleles, round(n,2)))
  Effective_Allele_size[[ms]] = round(n,1)
}
```

The interpretation of effective marker cardinality can be visualised by the probability of identity by chance as a function of effective marker cardinality. There is little gain for each incremental allele size above 4.

```{r Nstar_versus_IBC, echo=FALSE}
xs = seq(2,10,by=.01)
ys = (1/xs)*(1/(xs-1))
par(las=1, bty='n')
plot(x = xs, y = ys, type='l',lwd=3, yaxt ='n',ylim = c(0,0.5), xaxt='n',
     xlab = 'effective marker cardinality', panel.first =grid(),
     ylab='probability of identity by chance')
axis(1, at = 2:10)
axis(2, at= c(0.5, .3,.1,0))
abline(h=0, lty=2)
```
