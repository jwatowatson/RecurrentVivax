...
summary(mod)
xs=seq(0,4,by=.01)
plot(BPD_data$CPMQ, BPD_data$Failure)
lines(xs, predict(mod, newdata = data.frame(CPMQ=xs)), col='red', lwd=3)
```

So there is a predictive effect of carboxy primaquine on drug failure. 
This isn't very robust to the cutoff threshold for failure.

```{r}
Combined_Time_Data$Failure_YN = as.numeric(Combined_Time_Data$Failure > 0.5)
mod1 = glmer(Failure_YN ~ log10_carboxyPMQ + (1 | patientid), 
             data = Combined_Time_Data, family='binomial')
summary(mod1)
if(CREATE_PLOTS){
  par(las=1, bty='n')
  plot(Combined_Time_Data$log10_carboxyPMQ, 
       Combined_Time_Data$Failure_YN,pch='+',
       xlab = 'Day 7 Carboxy PMQ (ng/mL)', 
       ylab = 'Failure probability', xaxt ='n')
  axis(1, at = 1:3, labels = 10^(1:3))  
...
thresholds = seq(0.05,0.95,by=.01)
pvals=array(length(thresholds))
for(i in 1:length(thresholds)){
  Combined_Time_Data$Failure_YN = as.numeric(Combined_Time_Data$Failure > thresholds[i])
  mod1 = glmer(Failure_YN ~ log10_carboxyPMQ + (1 | patientid), 
               data = Combined_Time_Data, family='binomial')
  x=summary(mod1)
  pvals[i] = x$coefficients[,4][2]
}
plot(thresholds,pvals,type='l', xlab='Threshold value',ylab='P-value')
abline(h=0.05, col='red')
```


Who are the two individuals contributing the most to this signal?

```{r}
ind = which(Combined_Time_Data$Failure>0.5 & Combined_Time_Data$log10_carboxyPMQ<1.1)
Combined_Time_Data$patientid[ind]
```
...
mod_2D6 = lmer(log10_carboxyPMQ ~ ASscore + (1 | patientid), 
               data = Combined_Time_Data)
summary(mod_2D6)
plot(Combined_Time_Data$ASscore, Combined_Time_Data$log10_carboxyPMQ, pch=20)
lines(xs, predict(mod_2D6, data.frame(ASscore=xs,patientid='new'),allow.new.levels=T),lwd=3)
```


```{r}
Combined_Time_Data = filter(Combined_Time_Data, !is.na(ASscore), !Censored)
for(id in unique(Combined_Time_Data$patientid)){
  ind = Combined_Time_Data$patientid==id
  Combined_Time_Data$Failure_YN[ind] = max(Combined_Time_Data$Failure_YN[ind])
}
Combined_Time_Data = Combined_Time_Data[!duplicated(Combined_Time_Data$patientid),]
