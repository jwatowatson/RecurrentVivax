...
if(CREATE_PLOTS){
  ...
  for(i in 1:length(ordered$x)){
    if(diff(CI[i,]) > 0.005) segments(i,CI[i,1],i,CI[i,2], 
                                      col=drug_cols2[Thetas_BPD$drug[reLapse_ordered$ix[i]]])
  }
  ...
  for(i in 1:length(ordered$x)){
    ...
    points(x_time,
           Thetas_BPD$L[kk], pch=20, col = drug_cols2[Thetas_BPD$drug[reLapse_ordered$ix[i]]])
    
    if(diff(CI[i,]) > 0.005) segments(x_time,CI[i,1],x_time,CI[i,2], 
...
if(CREATE_PLOTS){
  ...
  # legend('topright', col = c(mycols_states_fg[c(2,1,3)], drug_cols[2:3]), 
  #        pch = c(15,1,17,NA,NA), cex = 0.7, bty = 'n', y.intersp = 0.75, 
  #        lty = c(NA,NA,NA,1,1), lwd = c(NA,NA,NA,1,1),
  #        legend = c('Classification uncertain','Classified relapse','Classified reinfection',
  #                   'No PMQ', 'PMQ'))
  
  legend('topright', col = c(mycols_states_fg[c(3,2,1)]), 
         pch = c(1,15,17), cex = 0.7, bty = 'n', y.intersp = 0.75, title = 'Classification',
         legend = c('Relapse','Uncertain','Reinfection'))
  
  legend('top', col = drug_cols[2:3], 
         cex = 0.7, bty = 'n', y.intersp = 0.75, 
         lty = 1, lwd = 1,
         legend = c('No PMQ', 'PMQ'))
}
```
