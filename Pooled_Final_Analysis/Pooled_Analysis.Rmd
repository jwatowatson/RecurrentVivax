...
# runs time agnostic (with radnom allale frequency draws) whose output used for genetic only efficacy estimate
RUN_MODELS_FULL_POSTERIOR_BPD_TAGNOSTIC = F # can do on laptop (45 mins)
...
if(RUN_MODELS_FULL_POSTERIOR_SIMPLE){
  ...
  Thetas_full_post = foreach(ss = 1:Ksamples, .combine = cbind) %do% {
    ...
    # run the model
    thetas_9MS = post_prob_CLI(MSdata = MS_pooled, Fs = Fs_random, p = p, cores = 42, verbose = F) 
    thetas_9MS$Episode_Identifier = rownames(thetas_9MS)
    thetas_9MS
  }
...
if(RUN_MODELS_FULL_POSTERIOR_BPD_TAGNOSTIC){
  ...
  # microsatellite allele frequnecy posterior
  #===============================================
  ...
  Thetas_non_random = post_prob_CLI(MSdata = MS_pooled[grepl('BPD', MS_pooled$ID),], Fs = Fs_Combined, cores = 42, verbose = F) 
...
hist(MS_final$timeSinceEnrolment[MS_final$Plotting_col_Values==1 &
     ...
     main = 'No PMQ', xlab = 'Months from start of study', xaxt='n',
...
```{r}
MS_main = MSs_all[1:3] # 1) Extract names of the core MSs
MS_add = MSs_all[4:length(MSs_all)] # Extract names of additionally MSs
MS_typed = MS_pooled_summary # Rename before relaplacing allele information with binary
MS_typed[,MSs_all][!is.na(MS_typed[,MSs_all])] = 1 # Replace allele information with binary
MS_main_count = length(MS_main) - rowSums(is.na(MS_typed[,MS_main])) # Extract number of markers typed per episode
MS_typed_count = length(MSs_all) - rowSums(is.na(MS_typed[,MSs_all])) # Extract number of markers typed per episode
MS_add_count = length(MS_add) - rowSums(is.na(MS_typed[,MS_add])) # Extract the number of additional markers typed per episode
names(MS_typed_count) = names(MS_add_count) = names(MS_main_count) = MS_typed$Episode_Identifier # Name by episode
BPD_ind = grepl('BPD_', MS_typed$ID) # Index for BPD individuals
VHX_ind = grepl('VHX_', MS_typed$ID) # Index for VHX individuals
```
...
#==========================================================================
#table(MS_typed_count) # numbers of samples partioned by number of markers successfully typed
#table(MS_add_count) # numbers of samples partioned by number of additional markers successfully typed
#table(MS_main_count) # numbers of samples partioned by number of core markers successfully typed
#MS_add_count[names(which(MS_main_count < 3))] # The number additional for those with 1 or more failed core - ask MALLIKA

writeLines(sprintf('Number of enrolment samples genotyped at additional markers: %s of %s (%s percent, %s VHX, %s BPD)',
                   sum(MS_typed$Episode[MS_add_count > 0] == 1),
                   sum(MS_typed$Episode == 1), 
                   round(100*sum(MS_typed$Episode[MS_add_count > 0] == 1)/sum(MS_typed$Episode == 1)), 
                   sum(MS_typed$Episode[VHX_ind][MS_add_count[VHX_ind] > 0] == 1), # VHX
                   sum(MS_typed$Episode[BPD_ind][MS_add_count[BPD_ind] > 0] == 1))) # BPD

writeLines(sprintf('Number of recurrent samples genotyped at additional markers: %s of %s (%s percent, %s VHX, %s BPD)',
                   sum(MS_typed$Episode[MS_add_count > 0] != 1),
                   sum(MS_typed$Episode != 1), 
                   round(100*sum(MS_typed$Episode[MS_add_count > 0] != 1)/sum(MS_typed$Episode != 1)), 
                   sum(MS_typed$Episode[VHX_ind][MS_add_count[VHX_ind] > 0] != 1), # VHX
...
# table(MS_add_count[MS_add_count > 0]) # Of samples genotyped at additional: number of additional successful

writeLines(sprintf('Of those genotyped at additional markers, number successfully genotyped at all six: %s of %s (%s percent, %s VHX, %s BPD)',
                   sum(MS_add_count[MS_add_count > 0] == 6), 
                   sum(MS_add_count > 0),
                   round(100*sum(MS_add_count[MS_add_count > 0] == 6)/sum(MS_add_count > 0)), 
                   sum(MS_add_count[VHX_ind][MS_add_count[VHX_ind] > 0] == 6),
                   sum(MS_add_count[BPD_ind][MS_add_count[BPD_ind] > 0] == 6)
                   )) 
```
...
```{r}
# Check that the episodes missing from MS_final are only those belong to the six most complex: yes
inds = MS_typed$Episode[which(!MS_typed$Episode_Identifier %in% MS_final$Episode_Identifier)] > 1 # Six too complex
# MS_typed$Episode_Identifier[which(!MS_typed$Episode_Identifier %in% MS_final$Episode_Identifier)][inds] 
...
#==========================================================================
ind_classified = MS_final$Episode_Identifier %in% names(MS_typed_count)
recur_count = MS_typed_count[MS_final$Episode_Identifier[ind_classified]]
...
#==========================================================================
names_additional = names(MS_add_count[MS_add_count > 0]) # Names of all additionally typed (inc. )
ind_add_classified = MS_final$Episode_Identifier %in% names_additional
recur_add_count = MS_add_count[MS_final$Episode_Identifier[ind_add_classified]]
