...
if(length(new.pkg) > 0){stop('Please run the script Install_required_packages.R before returning to this script. Thanks.')}

require(dplyr)      # filter, arrange, etc
...
require(igraph)     # For igraph
require(RColorBrewer) # Nice colours
require(knitr)      # for kable()
require(lme4)       # mixed effects linear model
...
if(RUN_MODELS_FULL_POSTERIOR_INFLATED){  
  ...
  Ksamples = 5# length(grep('C',colnames(Post_samples_matrix)))
...
```{r, echo=FALSE}
table(MS_final$Treatment[!duplicated(MS_final$ID)]) # 208 individuals by treatment
