...
RUN_MODELS_FALSE_POSITIVE = F           # only cluster!! (1 week)
# generates plots
CREATE_PLOTS = T
...
if(CREATE_PLOTS){
  ...
  for(i in 1:length(ids)){
    ...
    if(id == 'VHX_235' | id == 'BPD_27'){
      ...
      points(MS_final$timeSinceEnrollment[ind], 
             rep(i,length(ind)), 
             pch=1,cex = 1.5, 
             col='black')
    }
    ...
    points(MS_final$timeSinceEnrollment[ind], rep(i,length(ind)), 
...
if(CREATE_PLOTS){
  ...
  polygon(x = c(0,max(MS_final$timeSinceLastEpisode),
                max(MS_final$timeSinceLastEpisode),0),
          y = c(Epsilon_lower,Epsilon_lower,Epsilon_upper,Epsilon_upper),
          col = transparent_pink_band, border = NA)
