...
Under a discrete uniform distribution over $n$ alleles, the probability of seeing the same alleles twice by chance is simply $1 - \frac{1}{n(n-1)}$ (1 minus the probability of seeing two different ones).
Therefore, to calculate the effective marker cardinality, we solve the quadratic $x = 1 - \frac{1}{n(n-1)}$ for n, which has a real solution at $n = \frac{1+\sqrt{1+4(\frac{1}{1-x})}}{2}$.
...
<!-- James, above x is mathematically defined as the probability of seeing two the same (i.e. sum(draw1 == draw2)/N). In the code it is the probability of them being different (i.e. sum(draw1 != draw2)/N =  1- sum(draw1 == draw2)/N)  -->

Let the probability of drawing any two different alleles be $x = \frac{1}{n(n-1)}$ s.t. $n^2 - n - \dfrac{1}{x} = 0$ and $n = \frac{1+\sqrt{1+\frac{4}{x}}}{2}$.
...
for(ms in MSs_all){
  draw1 = sample(x = names(Fs_Combined[[ms]]), replace = T, size = N, prob = Fs_Combined[[ms]])
  draw2 = sample(x = names(Fs_Combined[[ms]]), replace = T, size = N, prob = Fs_Combined[[ms]])
  x_same = sum(draw1 == draw2)/N # prob of same
  x_diff = sum(draw1 != draw2)/N # prob of different
  n1 = (1 + sqrt(1+(4/x_diff)))/2  # if x is prob of different
  n2 = (1 + sqrt(1+(4/(1-x_same))))/2  # if x is prob of different
  print(n1 == n1) # Always the same
  writeLines(sprintf('The effective marker cardinality for %s is %s', ms, round(n1,2)))
  Effective_Allele_size[[ms]] = round(n,1)
}
```
