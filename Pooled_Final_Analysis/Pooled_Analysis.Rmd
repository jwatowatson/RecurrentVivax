...
```{r}
# Is there a significant difference beteen VHX that where geneticall typed and not? 
# Some odd warnings? 
...
Typed_VHX_epi_count =  table(MS_pooled_summary$ID[grep('VHX',MS_pooled$Episode_Identifier)])
Untyped_VHX_epi_count = All_VHX_epi_count[!names(All_VHX_epi_count) %in% names(Typed_VHX_epi_count)]

hist(Untyped_VHX_epi_count, col = 'lightgray', 
     main='VHX subset', xlab = 'Number of episodes', 
     breaks = seq(0.5,15.5,by=1), freq = F)
hist(Typed_VHX_epi_count,
     breaks = seq(0.5,15.5,by=1), add=T, 
     freq = F, col='red',density = 15)

# Bootstap test of difference in median
x1 = Typed_VHX_epi_count
x2 = Untyped_VHX_epi_count
theta_hat = median(x1) - median(x2)
B = 1000
theta_hat_boot = sapply(1:B, function(x){
...
percentile_95CI = quantile(theta_hat_boot, probs = c(0.025, 0.975)) # Significant
pivotal_95CI = 2*theta_hat - percentile_95CI # Significant
```
