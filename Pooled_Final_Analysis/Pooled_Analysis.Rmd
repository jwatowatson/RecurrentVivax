...
EXCLUDE_COMPLEX = F # There are 9 complex ones that considerably increase computation time
if(EXCLUDE_COMPLEX) {
...
MS_pooled$PMQ_partner = NA
for(i in 1:nrow(MS_pooled)){
  MS_pooled$PMQ_partner[i] = unique(Combined_Time_Data$PMQ_partner[Combined_Time_Data$patientid==MS_pooled$ID[i]])
}
table(MS_pooled$PMQ_partner[!duplicated(MS_pooled$ID) & 
                            1:nrow(MS_pooled)%in% grep('BPD',MS_pooled$ID)])
