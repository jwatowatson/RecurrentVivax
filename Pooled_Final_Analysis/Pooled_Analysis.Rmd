...
# runs the genetic model with the time-to-event prior on those that can be directly computed
# with 100 random draws from both the time-to-event and allele frequency posterior 
RUN_MODELS_FULL_POSTERIOR = F #

# runs the genetic model without the time-to-event prior on those that can be directly computed
# with 100 random draws from the allele frequency posterior 
RUN_MODELS_FULL_POSTERIOR_TAGNOSTIC = F 

# runs the genetic model with the time-to-event prior for those that cannot be directly computed
# with 100 random draws from both the time-to-event and allele frequency posterior 
...
# XXX ADD SUMMARIES FOR SEX, AGE, FOLLOW UP, PARASITE DENSITIES OR LINK TO WHERE THEY CAN BE FOUND
...
hist(COIs$MOI[COIs$Enrollment]+1, freq = F, breaks = seq(0.5,4.5, by=1),
...
hist(COIs$MOI[!COIs$Enrollment & COIs$Drug=='PMQ']+1, freq = F, 
...
mod = glm(MOI ~ enrollment + drug, family = 'poisson', 
          data = data.frame(MOI=COIs$MOI, # need to do minus 1 so it's Poisson appropriate
...
if(any(grepl('P0', thetas_9MS$Phased))){
  writeLines('Some probablistic phasing did not converge')
}else{
  writeLines('Zero accounts of unconverged probablistic phasing')   
}
```
...
if(any(grepl('P0', thetas_9MS_Tagnostic$Phased))){
  writeLines('Some probablistic phasing did not converge')
}else{
  writeLines('Zero accounts of unconverged probablistic phasing')   
}
...
```{r, include=FALSE}
MS_inflated = filter(MS_pooled, ID %in% IDs_remaining)
MS_inflated = Inflate_into_pairs(MS_data = MS_inflated)
...
if(RUN_MODELS_FULL_POSTERIOR_INFLATED){  
  ...
  # Pull out the credible intervals
  CI_C = apply(Res_total[,grep('C', colnames(Res_total)), drop=F],1, 
               quantile, probs = c(lowerCI, 0.5, upperCI), na.rm=T)
  CI_L = apply(Res_total[,grep('L', colnames(Res_total)), drop=F],1, 
               quantile, probs = c(lowerCI, 0.5, upperCI), na.rm=T)
  CI_R = apply(Res_total[,grep('I', colnames(Res_total)), drop=F],1, 
               quantile, probs = c(lowerCI, 0.5, upperCI), na.rm=T)
  CIs = t(rbind(CI_C, CI_L, CI_R)) 
  colnames(CIs) = paste(rep(c('C', 'L', 'I'), each = 3), colnames(CIs), sep = '')#, 'Phased')
  
  
  Results_Inflated = as.data.frame(CIs)
  Results_Inflated$Phased = Res_total$Phased
  save(Results_Inflated, file = f_name)
} else {
  load(f_name)
}

table(Results_Inflated$Phased)
# XXX CURRENTLY DOES NOT WORK AS PHASED OR NOT NOT RECORDED
if(any(grepl('P0', Results_Inflated$Phased))){
  writeLines('Some probablistic phasing did not converge')
} else {
  writeLines('Zero accounts of unconverged probablistic phasing')
}
...
# XXX CURRENTLY DOES NOT WORK AS PHASED NOT RECORDED
if(any(grepl('P0', MS_final$Phased))){
  writeLines('Some probablistic phasing did not converge')
} else {
  writeLines('Zero accounts of unconverged probablistic phasing')}
```
...
if(CREATE_PLOTS){
  ...
  legend('topright', col = c(mycols_states_fg), 
         ...
         legend = c('Failure (relapse or recrudescence)','Uncertain','Reinfection'))
...
if(CREATE_PLOTS){
  ...
  MS_final = arrange(MS_final, desc(L_median))
  ...
  legend('bottomleft', col = drug_cols2[2:3], 
         pch = 20, cex = 1.2, bty = 'o', border = 'white',
         legend = c('No PMQ','PMQ+'), inset = 0.04)
...
writeLines(sprintf('The false-positive discovery rate of the genetic model is estimated as %s percent. 
                   ...
                   round(100*sum((Inflated_Results$L+Inflated_Results$C) > Epsilon_upper,
                                  na.rm = T)/sum(!is.na(Inflated_Results$L)),1),
                   nrow(Inflated_Results)))

writeLines(sprintf('The null data are more diverse than the real data. As such %s percent were probablistically phased without convergence', round(sum(grepl('P0', Inflated_Results$Phased))/nrow(Inflated_Results)*100)))  
```
...
writeLines(sprintf('Observed BPD recurrences estimated to be failures, based on genetic model only, in the %s individuals is %s%% (%s-%s).',
                   nrow(BPD_data), 
                   round(sum(thetas_9MS_Tagnostic$`L50%`[grep('BPD',thetas_9MS_Tagnostic$Episode_Identifier)]),2),
                   round(sum(thetas_9MS_Tagnostic$`L2.5%`[grep('BPD',thetas_9MS_Tagnostic$Episode_Identifier)]),2),
                   round(sum(thetas_9MS_Tagnostic$`L97.5%`[grep('BPD',thetas_9MS_Tagnostic$Episode_Identifier)]),2)))
