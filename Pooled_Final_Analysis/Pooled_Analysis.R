...
writeLines(sprintf('Number of individuals in VHX and BPD trials: %s and %s, respectively',
...
if(RUN_MODELS_FULL_POSTERIOR_SIMPLE){
  ...
  Thetas_full_post = foreach(ss = 1:Ksamples, .combine = cbind) %do% {
    ...
    # run the model
    thetas_9MS = post_prob_CLI(MSdata = MS_pooled, Fs = Fs_random, p = p, cores = Ncores, verbose = F) 
    thetas_9MS$Episode_Identifier = rownames(thetas_9MS)
    thetas_9MS
  }
...
if(RUN_MODELS_FULL_POSTERIOR_BPD_TAGNOSTIC){
  ...
  # microsatellite allele frequency posterior
  #===============================================
  ...
  Thetas = foreach(ss = 1:Ksamples, .combine = cbind) %do% {
    
    # Draw a sample of allele frequencies
    Fs_random = lapply(Alpha_Posteriors, rdirichlet, n=1)
    
    # Name alleles (imporatant since we use the haplotype labels to extract allele frequencies in Log_Pr_yn_Gab)
    for(i in 1:length(Fs_random)){names(Fs_random[[i]]) = 1:length(Fs_random[[i]])}
    
    # Run the model 
    thetas_9MS = post_prob_CLI(MSdata = MS_pooled[grepl('BPD', MS_pooled$ID),], Fs = Fs_random, cores = detectCores()-2, verbose = F) 
    thetas_9MS # return for %do%
  }
  ...
  CI_L = apply(Thetas[,grep('L', colnames(Thetas))],1, quantile, probs = c(0.025, 0.975), na.rm=T)
  CI_R = apply(Thetas[,grep('I', colnames(Thetas))],1, quantile, probs = c(0.025, 0.975), na.rm=T)
  CIs = t(rbind(CI_C, CI_L, CI_R)) 
  colnames(CIs) = paste(rep(c('C', 'L', 'I'), each = 2), colnames(CIs), sep = '')
  
  # Combine and save
  Tagnostic_BPD = cbind(Thetas_non_random, CIs)
  save(Tagnostic_BPD, file = '../RData/GeneticModel/Tagnostic_BPD.RData')
  toc()
} else {
...
if(CREATE_PLOTS){
  ...
  writeLines(sprintf('The weighted average of recurrences which are estimated to be failures is %s%%',
                     round(100*sum(Thetas_BPD$L + Thetas_BPD$C)/nrow(Thetas_BPD),2)))
  writeLines(sprintf('The weighted average of recurrences which are estimated to be relapses is %s%%',
                     round(100*sum(Thetas_BPD$L)/nrow(Thetas_BPD),2)))
  writeLines(sprintf('Number of BPD recurrences analysed: %s', nrow(Thetas_BPD)))
...
if(RUN_MODELS_FULL_POSTERIOR_INFLATED){  
  ...
  Res_total=foreach(ss = 1:Ksamples, .combine = cbind) %do% {
    ...
    # Name allele: important since we use the haplotype labels to extract allele frequencies in Log_Pr_yn_Gab
    for(i in 1:length(Fs_random)){names(Fs_random[[i]]) = 1:length(Fs_random[[i]])}
  ...
  save(Res_total, file = '../RData/LargeFiles/FullPosterior_INF.bigRData') # save for safety
  toc()
  # Summarise the results, all CIs are 90%
  # Save this summary
  C_cols = grep('C',colnames(Res_total))
  L_cols = grep('L',colnames(Res_total))
  I_cols = grep('I',colnames(Res_total))
  Results_Inflated = 
    data.frame(Episode_Identifier = rownames(Res_total),
               C_median = apply(Res_total[,C_cols],1,
                                quantile,probs = 0.5,na.rm=T),
               C_min = apply(Res_total[,C_cols],1,
                             quantile,probs = lowerCI,na.rm=T),
               C_max = apply(Res_total[,C_cols],1,
                             quantile,probs = upperCI,na.rm=T),
               L_median = apply(Res_total[,L_cols],1,
                                quantile,probs = 0.5,na.rm=T),
               L_min = apply(Res_total[,L_cols],1,
                             quantile,probs = lowerCI,na.rm=T),
               L_max = apply(Res_total[,L_cols],1,
                             quantile,probs = upperCI,na.rm=T),
               I_median = apply(Res_total[,I_cols],1,
                                quantile,probs = 0.5,na.rm=T),
               I_min = apply(Res_total[,I_cols],1,
                             quantile,probs = lowerCI,na.rm=T),
               I_max = apply(Res_total[,I_cols],1,
                             quantile,probs = upperCI,na.rm=T))
...
writeLines(sprintf('In primaquine treated individuals (BPD), the weighted average of recrudescences is %s (%s-%s), for %s recurrences',
                   round(100*sum(MS_final$C_median[ind_PMQ_BPD],na.rm=T)/length(ind_PMQ_BPD),1),
...
Data_missing_core = MS_typd[MS_typed$Episode_Identifier %in% Episodes_missing_core, ] # All data 
