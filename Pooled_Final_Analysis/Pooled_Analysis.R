...
## ----CompleteDataPlot, fig.height=7, fig.width=7-------------------------
if(CREATE_PLOTS){
...
if(RUN_MODELS_FALSE_POSITIVE){
  ...
  if(!"APC_MSdata.bigRData"%in%list.files()){
    # The pooled MS data from BPD and VHX
    load('../RData/GeneticModel/MSdata_PooledAnalysis.RData')
    tic()
    APC_MSdata = Make_All_Pairwise_Comparisons(MSdata = MS_pooled, ncores=42)
...
legend(x = 25, y = 0.7, bty='n', col = c(1, regimen_colors, 1,1),
       pch=c(1,3,4,NA,NA), lty = c(NA,NA,NA,2,1),lwd=c(NA,NA,NA,2,2),
