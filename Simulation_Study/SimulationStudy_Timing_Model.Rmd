...
for(p in params_interest){
  writeLines(sprintf('For parameter %s, %s%% are below the true parameter',p,100*sum(res$comparison_matrix[,which(p==params_interest),1]<
                                                                                       unique(res$comparison_matrix[,which(p==params_interest),2]))/Sim_Iterations))
