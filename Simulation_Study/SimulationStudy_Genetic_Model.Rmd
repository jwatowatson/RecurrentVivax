    ...
    for(pattern in unique(settings[jobs_cor,]$COI_pattern)){
      ...
      for(relationship in relationships){ # Iterate over simulation scenarions (types of data)
        ...
        Z = barplot(X, beside = BESIDES, col = mycols[1:3], ylim = c(0,1), 
                    ...
                    main = "", names.arg = Ms, border = FALSE)
        
        # Add individual points
        for(j in 1:length(JOBS_pattern)){
          ind <- thetas_all$setting == JOBS_pattern[j]
          
          for(i in 1:length(States)){
            points(x = jitter(rep(Z[i,j], sum(ind)), factor = 1), 
                   y = thetas_all[ind,States[i]], 
                   col= "black", pch = ".", cex = 1) 
            #print(sum(ind))
          }
        }
        ...
        #   ind <- thetas_all$setting == JOBS_pattern[j]
        # for(i in 1:length(States)){
        #   if(any(thetas_all[ind,States[i]] > 0) & length(unique(thetas_all[ind,States[i]]))>4){
        #     x = vioplot(x = thetas_all[ind,States[i]], at = Z[i,j], add = TRUE, 
        #                 col = adjustcolor('black', alpha.f = 0.1))
        #   }
        # }}
        
        title(main = paste0(letters[which(relationships == relationship)], ") ", relationship_names[relationship]), 
              adj = 0)
        ...
        if(BESIDES){
          ...
          for(state in States){
            segments(x0 = Z[state,], x1 = Z[state,], 
                     y0 = X[state,] - 2*sds[state,], y1 = X[state,] + 2*sds[state,])
          }
        }
        
        # Coverage
        coverages = sapply(1:length(JOBS_pattern), function(j){
          ind <- thetas_all$setting == JOBS_pattern[j]
          cov = sapply(States, function(state){
            lci = X[state,j] - 2*sds[state,j]
            uci = X[state,j] + 2*sds[state,j]
            mean(thetas_all[ind, state] > lci & thetas_all[ind, state] < uci)
          }) 
        })
