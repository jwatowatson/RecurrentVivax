...
Need to change the plotting code also
...
```{r}
SIM_DATA = T 
RUN_MODELS = T # How problematic is the current complexity? 
...
N_indivs = 2 # 1000 # Number of individuals
...
```{r}
# Aside, what happens when data are NAs?
# load data
load(sprintf('SimulationOutputs/Sim_Genetic_Data/MS_Data_Cardinality%s_M%s_COIs%s_%s.RData', 
             cardinality, settings$M[job], settings$COI_pattern[job], relationship))

sim_output$MS_data_sim[,3:5] = NA # Can now accept NA data thus return prior   

######################################################################
# Run the model on the data using default uniform prior over states
######################################################################
TH = post_prob_CLI(MSdata = sim_output$MS_data_sim, cores = 7, Fs = sim_output$FS, verbose = F)
...
if(RUN_MODELS){
  for(cardinality in cardinalities){ # iterate over cardinality of markers
    for(relationship in relationships){ # iterative over simulation scenario 
      ...
      thetas_all = foreach(job = jobs, .combine = rbind, # iterate over jobs parameter settings
                           .packages = c('dplyr','Matrix','gtools','igraph','matrixStats','doParallel')
      ) %do% { # parallisation happening inside the function
        ...
        MSnames = paste0('MS',1:settings$M[job],sep = '') # Reconstruct microsatellite names
        sim_output$MS_data_sim[,MSnames] = NA # Can now accept NA data thus return prior   
...
if(PLOT_RESULTS){
  ...
  for(pattern in unique(settings$COI_pattern)){
    ...
    for(cardinality in cardinalities){ # cardinalities
      ...
      for(relationship in names(relationship_names)){
        
        # Load data 
        fname = load(paste0('SimulationOutputs/Posterior_Probs_N*=',cardinality,'_',
                            relationship,'_EffectCOI.RData'))
