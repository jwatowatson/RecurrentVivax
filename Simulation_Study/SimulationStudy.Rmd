...
# constant heterozygosity for all markers. 4 is the average in our panel
N_alleles_Choices = c(3,4,5,10)
K_indivs = 1000
  ...
  for(N_alleles in N_alleles_Choices){
    for(related_type in c('Sibling','Stranger','Clone')){
      ...
      thetas_all = foreach(s = 1:JOBS, .combine = rbind, 
        ...
        TH = post_prob_CLI(MSdata = MS_data, cores = 7,
      ...
      fname = paste0('SimulationOutputs/Posterior_Probs_N*=',N_alleles,'_',
  ...
  for(N_alleles in N_alleles_Choices){
    ...
    for(State in c('C','I','L')){
      ...
      for(related_type in names(related_type_col)){
        ...
        if(State == 'C' & related_type == 'Clone'){
          mtext(text = 'Clonal scenario', side = 2, line = 4, las=3, cex=1, bg = 'blue')
        }
        if(State == 'C' & related_type == 'Stranger'){
          mtext(text = 'Stranger scenario', side = 2, line = 4, las=3, cex=1)
        }
        if(State == 'C' & related_type == 'Sibling'){
          mtext(text = 'Sibling scenario', side = 2, line = 4, las=3, cex=1)
        }
      }
    }
  }
