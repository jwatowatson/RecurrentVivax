...
data {
  ...
  int<lower=0,upper=N>        N_index[Neps];            // Index of individual groupings
  int<lower=0,upper=N_noPMQ>  N_noPMQ_index[N];         // Vector mapping the the individual ID (from 1 to N) to the random effects index: logit_p
  int<lower=0,upper=N_PMQ>    N_PMQ_index[N];           // Vector mapping the the individual ID (from 1 to N) to the random effects index: logit_p_PMQ
