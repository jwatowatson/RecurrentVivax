...
hist(1/thetas$lambda,freq = FALSE, xlim = c(300,1600), main='',
     xlab='Time to reinfection (1/lambda)', ylab = '', yaxt='n')
lines(dnorm(x = 1:1000,mean = Prior_params_M3$mu_inv_lambda, 
            sd = Prior_params_M3$sigma_inv_lambda),
      col='red',lwd=3)

# gamma: late relapse rate
hist(1/thetas$gamma,freq = FALSE, xlim = c(0,200), main='',
     xlab='Time to late relapse (1/gamma)', ylab = '', yaxt='n')
lines(dnorm(x = 1:1000,mean = Prior_params_M3$mu_inv_gamma, 
            sd = Prior_params_M3$sigma_inv_gamma),
      col='red',lwd=3)

# AS_shape (Weibull shape parameter for AS monotherapy)
hist(thetas$AS_shape, xlim = c(0,5), freq = F, main='',
     xlab= 'AS shape parameter', yaxt='n', ylab='')
lines(seq(0,10,by=0.1), dnorm(x = seq(0,10,by=0.1),
            mean = Prior_params_M3$mu_AS_shape, 
            sd = Prior_params_M3$sigma_AS_shape),
      col='red',lwd=3)

# AS_scale (Weibill scale parameter for AS monotherapy)
hist(thetas$AS_scale, xlim = c(15,35), freq = F, main='',
     xlab= 'AS scale parameter', yaxt='n', ylab='')
lines(seq(0,50,by=1), dnorm(x = seq(0,50,by=1),
            mean = Prior_params_M3$mu_AS_scale, 
            sd = Prior_params_M3$sigma_AS_scale),
      col='red',lwd=3)

# CQ_shape (Weibull shape parameter for CQ with or without PMQ)
hist(thetas$CQ_shape, xlim = c(0,6), freq = F, main='',
     xlab= 'CQ shape parameter', yaxt='n', ylab='')
lines(seq(0,10,by=0.1), dnorm(x = seq(0,10,by=0.1),
            mean = Prior_params_M3$mu_CQ_shape, 
            sd = Prior_params_M3$sigma_CQ_shape),
      col='red',lwd=3)

# CQ_scale (Weibull scale parameter for CQ with or without PMQ)
hist(thetas$CQ_scale, xlim = c(35,55), freq = F, main='',
     xlab= 'CQ scale parameter', yaxt='n', ylab='')
lines(seq(30,70,by=1), dnorm(x = seq(30,70,by=1),
            mean = Prior_params_M3$mu_CQ_scale, 
            sd = Prior_params_M3$sigma_CQ_scale),
      col='red',lwd=3)

# Mean logit p (hierachical mean reinfection proportion)
hist(thetas$logit_mean_p, xlim = c(-4,-1), freq = F, main='',
     xlab= 'Population logit p', yaxt='n', ylab='')
lines(seq(-5,3,by=.01), dnorm(x = seq(-5,3,by=.01),
            mean = Prior_params_M3$Hyper_logit_mean_p, 
            sd = Prior_params_M3$Hyper_logit_sd_p),
      col='red',lwd=3)

# Mean logit p_PMQ (hierachical mean reinfection proportion after PMQ)
hist(thetas$logit_mean_p_PMQ, xlim = c(2,5), freq = F, main='',
     xlab= 'Population logit p_PMQ', yaxt='n', ylab='')
lines(seq(0,5,by=.01), dnorm(x = seq(0,5,by=.01),
            mean = Prior_params_M3$Hyper_logit_mean_p_PMQ, 
            sd = Prior_params_M3$Hyper_logit_sd_p_PMQ),
      col='red',lwd=3)

# Mean c1 AS (hierachical mean early relapse)
hist(thetas$logit_c1_AS, xlim = c(-5,-2), freq = F, main='',
     xlab= 'Population logit c1: AS', yaxt='n', ylab='')
lines(seq(-5,5,by=.01), dnorm(x = seq(-5,5,by=.01),
            mean = Prior_params_M3$Hyper_logit_c1_mean, 
            sd = Prior_params_M3$Hyper_logit_c1_sd),
      col='red',lwd=3)

# Mean c1: CQ (hierachical mean early relapse)
hist(thetas$logit_c1_CQ,  xlim = c(-5,-2), freq = F, main='',
...
hist(thetas$logit_c1_CQ_PMQ,  xlim = c(-5,-2), freq = F, main='',
...
hist(thetas$logit_EarlyL, xlim = c(-1,1), freq = F, main='',
     xlab= 'logit EarlyL', yaxt='n', ylab='')
lines(seq(-1,5,by=.01), dnorm(x = seq(-1,5,by=.01),
            mean = Prior_params_M3$Early_L_logit_mean, 
            sd = Prior_params_M3$Early_L_logit_sd),
      col='red',lwd=3)
```

![](TimingModelStan_files/figure-html/prior_to_posterior-1.png)<!-- -->
