...
#load('~/Documents/GitHub/RecurrentVivax/RData/TimingModel/MOD3_theta_estimates.RData')
load('../../../RecurrentVivax/RData/TimingModel/MOD3_theta_estimates.RData')

source("../Probabilistic_Model_Functions/Data_functions.R")
source('../Probabilistic_Model_Functions/iGraph_functions.R')
source("../Probabilistic_Model_Functions/post_prob_CLI.R") 
source("../Probabilistic_Model_Functions/test_Rn_compatible.R") 
source("../DataPreparation/Data_Inflation_Functions.R")
source('../Plotting MS Data/PlottingFunction.R')

# The pooled MS data from BPD and VHX
load('../../RData/MS_data_PooledAnalysis.RData')
```
...
```{r}
# get rid of the episodes greater than 3
# Aimee: if you comment out the next line then you'll see the bug in the next code section (running the model)
# MS_data_reformated = filter(MS_data_reformated, !(Episode > 3))
...
These dataframes are sorted by episode number so the columns correspond between them

```{r}
Theta_Matrix = data.frame(do.call(rbind, thetas_9MS))
Theta_Matrix$Episode_Identifier = rownames(Theta_Matrix)
Theta_Matrix = arrange(Theta_Matrix, Episode_Identifier)

Theta_Matrix_Agnostic = data.frame(do.call(rbind, thetas_9MS_Tagnostic))
Theta_Matrix_Agnostic$Episode_Identifier = rownames(Theta_Matrix_Agnostic)
Theta_Matrix_Agnostic = arrange(Theta_Matrix_Agnostic, Episode_Identifier)

Time_Estimates_1 = filter(Mod3_ThetaEstimates, 
                          Episode_Identifier %in% Theta_Matrix$Episode_Identifier)
Time_Estimates_1 = arrange(Time_Estimates_1, Episode_Identifier)

Theta_Matrix$drug = as.factor(Time_Estimates_1$arm_num)
Theta_Matrix_Agnostic$drug = as.factor(Time_Estimates_1$arm_num)
```
...
```{r}
pmq_ind = Time_Estimates_1$arm_num == 'CHQ/PMQ'
plot((Theta_Matrix_Agnostic$L[pmq_ind]), (Theta_Matrix$L[pmq_ind]), col=Theta_Matrix$drug)
plot((Theta_Matrix_Agnostic$L[!pmq_ind]), (Theta_Matrix$L[!pmq_ind]), col=Theta_Matrix$drug)
plot(log10(Time_Estimates_1$Relapse_mean_theta), log10(Theta_Matrix$L),col=as.factor(Time_Estimates_1$arm_num))
lines(-10:10,-10:10)

plot(log10(Time_Estimates_1$ReInfection_mean_theta), log10(Theta_Matrix$I),col=as.factor(Time_Estimates_1$arm_num))
lines(-10:10,-10:10)
```


Currently all the recrudescence probabilities are exactly 0...
Is this the thing you were worried about?

Other problem: the complex ones (for which it returns NA) are not grouped by ID in the function output

Plot for the relapse states

```{r}
plot(log10(Time_Estimates_1$Relapse_mean_theta), 
     log10(Theta_Matrix$L), col = as.factor(Time_Estimates_1$arm_num),
     xlab='prior for L', ylab = 'posterior for L')
title('Probabilities of Relapse')
lines(-10:10,-10:10)
```

Plot for the reinfection states

```{r}
plot(log10(Time_Estimates_1$ReInfection_mean_theta), 
     log10(Theta_Matrix$I), col = as.factor(Time_Estimates_1$arm_num), 
     xlab='prior for I', ylab = 'posterior for I')
title('Probabilities of Reinfection')
lines(-10:10,-10:10)
```

Is this not strange: (?)
WHy is the time agnostic model giving some of them such low probabilities?

```{r}
plot(log10(Theta_Matrix_Agnostic$L), log10(Theta_Matrix$L),
     col = as.factor(Theta_Matrix_Agnostic$drug))

ids1 = Time_Estimates_1$patientid[which(log10(Theta_Matrix_Agnostic$L) < -4)]
#View(MS_pooled[MS_pooled$ID %in% ids1,])

Spec_case = filter(MS_pooled, ID=='VHX_485')
post_prob_CLI(MS_data = Spec_case,Fs = Fs_Combined)
```
...
```{r}
tic()
Res = post_prob_CLI(MS_data = MS_pooled_pairs, Fs = Fs_Combined, p = P_matrix, cores = 7)
